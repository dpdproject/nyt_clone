...
export default function Business() {
    ...
    const subsection = business.map(item => item.subsection)
    ...
    const otherArticles = business.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/business/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
