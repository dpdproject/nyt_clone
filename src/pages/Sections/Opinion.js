...
export default function Opinion() {
    ...
    const opinion = news.opinion;

    // CREATE SUBSECTION ARRAY
    const subsection = opinion.map(item => item.subsection)
    ...
    const otherArticles = opinion.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/opinion/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
