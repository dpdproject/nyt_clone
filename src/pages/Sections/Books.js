...
export default function Books() {
    ...
    const subsection = books.map(item => item.subsection)
    ...
    const otherArticles = books.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/books/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
