...
export default function Food() {
    ...
    const subsection = food.map(item => item.subsection)
    ...
    const otherArticles = food.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/food/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
