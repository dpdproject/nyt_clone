...
export default function World() {
    ...
    const subsection = world.map(item => item.subsection)
    ...
    const tempWorld = world.filter(item => item.abstract !== "")
    const firstFour = tempWorld.slice(0, 4);

    const title = firstFour.map(item => item.title);
    const section = world.map(item => item.section)
    ...
    const otherArticles = world.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        return (
            <li>
                <Link to={`/section/world/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
