...
export default function Arts() {
    ...
    const subsection = arts.map(item => item.subsection)
    ...
    const otherArticles = arts.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/arts/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
