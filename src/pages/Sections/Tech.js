...
export default function Tech() {
    ...
    const subsection = tech.map(item => item.subsection)
    ...
    const otherArticles = tech.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/tech/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
