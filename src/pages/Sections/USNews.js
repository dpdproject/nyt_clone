...
export default function USNews() {
    ...
    const subsection = us.map(item => item.subsection)
    ...
    const otherArticles = us.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        return (
            <li>
                <Link to={`/section/us/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
