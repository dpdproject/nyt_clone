...
export default function Style() {
    ...
    const subsection = style.map(item => item.subsection)
    ...
    const otherArticles = style.slice(4, 25);
    ...
    const subsections = filteredSub.map(item => {
        ...
        return (
            <li>
                <Link to={`/section/style/${item}`} key={item.uri}>{item}</Link>
            </li>
        )
    })
